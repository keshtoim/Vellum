...
namespace PublishingHouseApp
{
    ...
    public class EditorForm : RoleFormBase
    {
        ...
        private void BuildAuthorsPanel()
        {
            ...
            btnDel.Click+=(s,e)=>{if(grid.SelectedRows.Count==0){UIHelper.ShowError("Выберите автора.");return;}var r=grid.SelectedRows[0];if(!UIHelper.Confirm($"Удалить «{r.Cells["surname"].Value} {r.Cells["name"].Value}»?"))return;int res=DatabaseHelper.ExecuteNonQuery("DELETE FROM Author WHERE author_id=@id",new[]{new SqlParameter("@id",Convert.ToInt32(r.Cells["author_id"].Value))});if(res>=0){sb.Refresh();editPanel.Visible=false;}};
            WireDoubleClick(grid,btnEdit);
            sb.Init(grid,(kw,sc)=>{var dt=DatabaseHelper.ExecuteQuery($"SELECT author_id,surname,name,patronymic,email,phone,tax_id FROM Author WHERE surname LIKE @k OR name LIKE @k OR email LIKE @k ORDER BY {sc}",new[]{new SqlParameter("@k",$"%{kw}%")});HideCols(grid,"author_id");SetHeaders(grid,new[]{("surname","Фамилия"),("name","Имя"),("patronymic","Отчество"),("email","Email"),("phone","Телефон"),("tax_id","ИНН")});return dt;},new[]{"Фамилия","Имя","Email"},new[]{"surname","name","email"});
        }
        ...
        private void BuildContractsPanel()
        {
            ...
            btnDel.Click+=(s,e)=>{if(grid.SelectedRows.Count==0){UIHelper.ShowError("Выберите договор.");return;}var r=grid.SelectedRows[0];if(!UIHelper.Confirm($"Удалить договор №{r.Cells["contract_id"].Value}?"))return;int res=DatabaseHelper.ExecuteNonQuery("DELETE FROM Contract WHERE contract_id=@id",new[]{new SqlParameter("@id",Convert.ToInt32(r.Cells["contract_id"].Value))});if(res>=0){sb.Refresh();editPanel.Visible=false;}};
            WireDoubleClick(grid,btnEdit);
            sb.Init(grid,(kw,sc)=>{var dt=DatabaseHelper.ExecuteQuery($"SELECT c.contract_id,c.signing_date,c.valid_until,c.amount,a.surname+' '+a.name AS author,c.author_id FROM Contract c JOIN Author a ON a.author_id=c.author_id WHERE a.surname LIKE @k OR a.name LIKE @k ORDER BY {sc}",new[]{new SqlParameter("@k",$"%{kw}%")});HideCols(grid,"contract_id","author_id");SetHeaders(grid,new[]{("signing_date","Дата подписания"),("valid_until","Действует до"),("amount","Сумма"),("author","Автор")});return dt;},new[]{"Дата","Сумма","Автор"},new[]{"c.signing_date","c.amount","a.surname"});
        }
        ...
        private void BuildPublicationsPanel()
        {
            ...
            btnDel.Click+=(s,e)=>{if(grid.SelectedRows.Count==0){UIHelper.ShowError("Выберите издание.");return;}var r=grid.SelectedRows[0];if(!UIHelper.Confirm($"Удалить «{r.Cells["title"].Value}»?"))return;int res=DatabaseHelper.ExecuteNonQuery("DELETE FROM Publication WHERE publication_id=@id",new[]{new SqlParameter("@id",Convert.ToInt32(r.Cells["publication_id"].Value))});if(res>=0){sb.Refresh();editPanel.Visible=false;}};
            WireDoubleClick(grid,btnEdit);
            sb.Init(grid,(kw,sc)=>{var dt=DatabaseHelper.ExecuteQuery($"SELECT p.publication_id,p.title,p.isbn,p.contract_id,p.type_id,p.subject_id,p.class_id,t.type_name,s.subject_name,cl.class_level,a.surname+' '+a.name AS author FROM Publication p LEFT JOIN Type t ON t.type_id=p.type_id LEFT JOIN Subject s ON s.subject_id=p.subject_id LEFT JOIN Class cl ON cl.class_id=p.class_id LEFT JOIN Contract c ON c.contract_id=p.contract_id LEFT JOIN Author a ON a.author_id=c.author_id WHERE p.title LIKE @k OR p.isbn LIKE @k OR a.surname LIKE @k ORDER BY {sc}",new[]{new SqlParameter("@k",$"%{kw}%")});HideCols(grid,"publication_id","contract_id","type_id","subject_id","class_id");SetHeaders(grid,new[]{("title","Название"),("isbn","ISBN"),("type_name","Тип"),("subject_name","Тематика"),("class_level","Класс"),("author","Автор")});return dt;},new[]{"Название","ISBN","Автор"},new[]{"p.title","p.isbn","a.surname"});
        }
        ...
        private void BuildStagesPanel()
        {
            ...
            AddSaveCancel(scroll,ref y,
                onSave:()=>{if(string.IsNullOrWhiteSpace(tbNm.Text)||cbSt.SelectedIndex<0||cbPu.SelectedItem==null){UIHelper.ShowError("Заполните все поля.");return;}int ni=cbSt.SelectedIndex;if(eid!=-1){if(ni<pi){UIHelper.ShowError("Нельзя вернуть статус назад.");return;}if(ni>pi+1){UIHelper.ShowError($"Следующий: «{Statuses[pi+1]}»");return;}}int pid=((ComboItem)cbPu.SelectedItem).Id;var prm=new[]{new SqlParameter("@n",tbNm.Text.Trim()),new SqlParameter("@d",dtpSt.Value.Date),new SqlParameter("@s",Statuses[ni]),new SqlParameter("@p",pid)};int res=eid==-1?DatabaseHelper.SmartInsert("PreparationStage","INSERT INTO PreparationStage (stage_name,start_date,status,publication_id) VALUES (@n,@d,@s,@p)",prm):DatabaseHelper.ExecuteNonQuery("UPDATE PreparationStage SET stage_name=@n,start_date=@d,status=@s,publication_id=@p WHERE stage_id=@id",Append(prm,new SqlParameter("@id",eid)));if(res>=0){sb.Refresh();editPanel.Visible=false;}},
                onCancel:()=>editPanel.Visible=false);
            ...
            btnDel.Click+=(s,e)=>{if(grid.SelectedRows.Count==0){UIHelper.ShowError("Выберите этап.");return;}var r=grid.SelectedRows[0];if(!UIHelper.Confirm($"Удалить «{r.Cells["stage_name"].Value}»?"))return;int res=DatabaseHelper.ExecuteNonQuery("DELETE FROM PreparationStage WHERE stage_id=@id",new[]{new SqlParameter("@id",Convert.ToInt32(r.Cells["stage_id"].Value))});if(res>=0){sb.Refresh();editPanel.Visible=false;}};
            WireDoubleClick(grid,btnEdit);
            sb.Init(grid,(kw,sc)=>{var dt=DatabaseHelper.ExecuteQuery($"SELECT ps.stage_id,ps.stage_name,ps.start_date,ps.status,p.title AS publication,ps.publication_id FROM PreparationStage ps JOIN Publication p ON p.publication_id=ps.publication_id WHERE ps.stage_name LIKE @k OR ps.status LIKE @k ORDER BY {sc}",new[]{new SqlParameter("@k",$"%{kw}%")});HideCols(grid,"stage_id","publication_id");SetHeaders(grid,new[]{("stage_name","Название"),("start_date","Дата начала"),("status","Статус"),("publication","Издание")});return dt;},new[]{"Название","Статус","Дата"},new[]{"ps.stage_name","ps.status","ps.start_date"});
        }
